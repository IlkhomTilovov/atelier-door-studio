-- Door frames as independent assets (not tied to a specific door)
-- Idempotent: safely undoes the previous (rejected) approach of frame-on-doors columns.
ALTER TABLE public.doors DROP COLUMN IF EXISTS frame_image_url;
ALTER TABLE public.doors DROP COLUMN IF EXISTS frame_scale;
